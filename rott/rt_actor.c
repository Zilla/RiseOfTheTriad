...
static byte dirdiff[8][8] = {{0, 1, 2, 3, 4, 3, 2, 1}, {1, 0, 1, 2, 3, 4, 3, 2}, {2, 1, 0, 1, 2, 3, 4, 3}, {3, 2, 1, 0, 1, 2, 3, 4}, {4, 3, 2, 1, 0, 1, 2, 3}, {3, 4, 3, 2, 1, 0, 1, 2}, {2, 3, 4, 3, 2, 1, 0, 1}, {1, 2, 3, 4, 3, 2, 1, 0}};

static byte dirorder[8][2] = {{southeast, northeast}, {east, north}, {northeast, northwest}, {north, west}, {northwest, southwest}, {west, south}, {southwest, southeast}, {south, east}};
...
static byte dirorder16[16][2] = {
    {15, 1}, {0, 2}, {1, 3}, {2, 4}, {3, 5}, {4, 6}, {5, 7}, {6, 8}, {7, 9}, {8, 10}, {9, 11}, {10, 12}, {11, 13}, {12, 14}, {13, 15}, {14, 0}};
...
void BeginEnemyFatality(objtype *ob, objtype *attacker)
{
   ...
   else
   {
      ...
      else if ((ob->obclass == blitzguardobj) &&
               (ob->state->condition & SF_DOWN))
      {
...
void T_Projectile(objtype *ob)
{
   ...
   else if ((ob->obclass == p_splitmissileobj) &&
   ...
   do
   {
      ...
      if (checksprite && (checksprite->flags & FL_BLOCK) && (condition == SHOOT) &&
          ...
          (InRange(orig, (objtype *)checksprite,
                   FindDistance(orig->x - checksprite->x, orig->y - checksprite->y)) == true))
         ...
         if (tempactor->which == ACTOR)
         {
            if ((abs(tempactor->z - z) < 32) && (!(tempactor->flags & FL_DYING)) &&
                ...
                (InRange(shooter, tempactor,
                         FindDistance(shooter->x - tempactor->x, shooter->y - tempactor->y)) == true))
            {
